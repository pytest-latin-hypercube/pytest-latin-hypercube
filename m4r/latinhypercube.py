...
def latin_hyperrectangle(args: dict, seed: int = 847, number_of_iterations: int = 1, max_iterations: int = 1000):
    ...
    for key, value in args.items():
        ...
        if len(value) > longest_parameter_space:
            longest_parameter_space = len(value)
    samples = []
    shuffled_args = {key: list(np.random.permutation(value*number_of_iterations)) for key, value in args.items()}
    iter = 0
    while len(samples) < longest_parameter_space*number_of_iterations:
        ...
        for key, value in args.items():
            if len(shuffled_args[key]) == 0:
                shuffled_args[key] = list(np.random.permutation(value*number_of_iterations))

        if iter > max_iterations:
            warnings.warn(f"Maximum number of iterations ({max_iterations}) reached. Returning partial sample.")
            break
    return keys, samples
